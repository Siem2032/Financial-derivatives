...
def hedge_delta_position(stock_id, options, stock_value):
    """
    Delta hedge the options portfolio by trading the underlying stock using aggressive IOC orders.
    Matches the exact order/notation from the provided arbitrage example.
    """
    positions = exchange.get_positions()
    delta = 0.0

    # ---- 1) Compute total delta ----
    for option_id, option in options.items():
        ...
        print(f"Option {option_id}: delta={option_delta:.4f}, "
              f"position={position}, delta*position={delta_position:.4f}")
    

    print(f"\nTotal portfolio delta (options): {delta:.4f}")


    stock_position = positions[stock_id]
    print(f"- The current position in the stock {stock_id} is {stock_position}.")

    POSITION_LIMIT = 100  # max allowed long/short stock position

    desired_stock_position = -delta  # want total delta = 0
    # Clamp to position limits
    desired_stock_position = max(-POSITION_LIMIT, min(POSITION_LIMIT, desired_stock_position))

    # ---- 3) Compute how many shares to trade ----
   
    delta_trade = desired_stock_position - stock_position  # positive -> buy, negative -> sell

    # Skip tiny trades
    if abs(delta_trade) < 1:
        print("Delta near zero or fully limited, no hedge needed.")
        return

    trade_volume = abs(int(delta_trade))  # integer number of shares

    if trade_volume == 0:
        print("Trade volume rounded to zero, skipping.")
        return
    ...
    if delta_trade > 0:
        # Need to buy stock => place aggressive bid slightly above best ask
        price = book.asks[0].price + TINY_TICK
        exchange.insert_order(
            stock_id,           # instrument_id first
            price=price,        # price second
            volume=trade_volume, 
            side="bid",         # "bid" for buy
            order_type="ioc"
        )
        print(f"Placing IOC bid: {trade_volume} shares at {price:.2f}")
    else:
        # Need to sell stock => place aggressive ask slightly below best bid
        price = book.bids[0].price - TINY_TICK
        exchange.insert_order(
            stock_id, 
            price=price, 
            volume=trade_volume, 
            side="ask",         # "ask" for sell
            order_type="ioc"
        )
        print(f"Placing IOC ask: {trade_volume} shares at {price:.2f}")
...
while True:
    ...
    if stock_value is None:
        print(
            "Empty stock order book on bid or ask-side, or both, unable to update option prices."
        )
        time.sleep(4)
        continue
